...
app.get("/api/favorites", (req, res) => {
  try {
    ...
    if (fs.existsSync(RATINGS_FILE))
      ratings = JSON.parse(fs.readFileSync(RATINGS_FILE, "utf-8"));
...
function matchRecipes(userIngredients, dietary, difficulty) {
  ...
  const scored = filtered.map((r) => {
    ...
    const matchPercentage =
      recipeIngredientsLower.length === 0
        ? 0
        : Math.round((matchedIngredients.length / recipeIngredientsLower.length) * 100);
